...
1. **Class it up**: Wrap a class around the last function. Instantiate the class and call the function. Then write tests.
1. **Output Ascending**: Generate n random integers from 0 to 100, then output them in ascending order.
1. **Sum Digits**: Generate a random number from 1000 to 9999 and output the sum of its digits. For example, if the number is 1049, then the output should be 14.
1. **Sum Digits w/drop**: Generate a random number from 1000 to 9999 and output the sum of its digits, repeatedly, dropping the left-most digit after each iteration, until there is only one digit only. For example, if the number is 1049, then the output should be 14, 13, 13, 9.
...
1. **Random String Test**: Write a function that takes two random 10-character strings as arguments, which returns True when the first string starts with the first two characters in the second string - and False otherwise.
